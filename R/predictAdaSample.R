...
predictAdaSample <- function(models, data, labelNames, classifier = "svm", prob = FALSE) {
  L = length(models)
  ...
  for (l in 1:L) {
    if (classifier == "svm") {
      tmp <- attr(predict(models[[l]], newdata=t(data), probability = TRUE), "prob")[,names(labelNames)]
      predictMat <- predictMat + tmp
    } else {
      tmp <- predict(models[[l]], newdata=t(data), type="prob")[,names(labelNames)]
      predictMat <- predictMat + tmp
    }
  }
  if(prob==TRUE) {
